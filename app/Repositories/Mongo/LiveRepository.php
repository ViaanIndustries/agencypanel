...
class LiveRepository extends AbstractRepository implements LiveInterface {
    ...
    public function __construct(
        PassbookInterface $passbookRep,  CustomerInterface $CustomerRepo    ){
    ...
    public function index_old($requestData, $perpage = NULL) {
        ...
        $query          = $this->model->with(array('leadcast' => function ($q) {$q->select('_id', 'first_name', 'last_name');}))->orderBy('schedule_at');
...
//        $created_at_end     =   (isset($requestData['created_at_end']) && $requestData['created_at_end'] != '') ? hyphen_date($requestData['created_at_end']) : '';
	...
	$data = [];
        $appends_array  = array('sort' => $sort);
        
        $query = \App\Models\Live::where('is_live',true)->where('is_end', false)
            ->with(array('artist' => function ($q) {
            $q->select('_id', 'picture','coins','stats','first_name','last_name','about_us','city');
        }));
   	...
   	if(isset($results))
	{
	    ...
	    {
		$timeFirst = strtotime($r['start_at']);
		...
		$r['duration']   = $differenceInSeconds;
		$r['reports'] = $this->getArtistReportCount($r['_id']);
		$total_report +=  $r['reports'];
		$data[] = $r;
